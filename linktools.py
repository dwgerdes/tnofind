...
def genNoise(exposure, nfakes=100):
    ...
    '''
    cat = Catalog(date=str,expnum=int,exptime=float, 
                   ra=float,dec=float,band=str,ccd=int,mag=float,nite=int,objid=int)
    ...
    for i in range(len(points)):
        ...
        if ccdNum>-99 and Nfiducial<nfakes:
            cat.append(ra=fake_ra, dec=fake_dec, date=exposure.UTobs, expnum=exposure.expnum,
                      exptime=exposure.exptime, band=exposure.band, ccd=ccdNum, objid=-999,
                       nite=exposure.nite)
            Nfiducial+=1
